    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">All QR codes for collections and standalone products</p>
        </div>
      ...
      {!qrCodes || qrCodes.length === 0 ? (
        <div className="card text-center py-14">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-1">No QR codes yet</p>
          <p className="text-sm text-slate-400 dark:text-slate-500 mb-5">Create a collection or product to generate one</p>
              ...
              <div key={qr.id} className="card-hover">
                <div className="flex items-start justify-between mb-3">
                  <div className="min-w-0">
                    <h3 className="font-semibold text-slate-900 dark:text-slate-100 truncate">{qr.label}</h3>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {displayName && <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{displayName}</p>}
                    </div>
                  </div>
                </div>

                {qr.qr_png_path && (
                  <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-600 rounded-xl p-5 mb-3 flex justify-center">
                ...
                <div className="text-xs text-slate-400 dark:text-slate-500 space-y-0.5 mb-3">
