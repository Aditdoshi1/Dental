      ...
      >
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-semibold text-slate-900 group-hover:text-brand-700 transition-colors">{c.title}</h3>
            {c.visibility === "personal" && <span className="badge-purple">Personal</span>}
            {!c.active && <span className="badge-gray">Inactive</span>}
          </div>
          <div className="flex items-center gap-2 mt-1.5">
            <span className="text-xs text-slate-500">{count} {count === 1 ? "product" : "products"}</span>
            <span className="text-xs text-slate-300">&middot;</span>
            <span className="text-xs text-slate-500">by {c.profiles?.display_name || "Unknown"}</span>
            <span className="text-xs text-slate-300">&middot;</span>
            <span className="text-xs text-slate-400">{formatDate(c.created_at)}</span>
          </div>
        </div>
        <svg className="w-4 h-4 text-slate-300 group-hover:text-brand-500 group-hover:translate-x-0.5 transition-all flex-shrink-0 ml-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div>
        ...
        {shopCollections.length === 0 ? (
          <div className="card text-center py-10">
            <div className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center mx-auto mb-3">
              <svg className="w-6 h-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
            </div>
            <p className="text-sm text-slate-500 mb-1">No shop-wide collections yet</p>
            <p className="text-xs text-slate-400 mb-4">Create one to share product recommendations with your team</p>
      ...
      <div>
        ...
        {personalCollections.length === 0 ? (
          <p className="text-sm text-slate-400">No personal collections</p>
        ) : (
          <div className="space-y-2">
